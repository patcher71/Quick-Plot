classdef QuickPlots_v1< matlab.apps.AppBase
...
T = T(mask, :);
if height(T) == 0
    % Nothing to plot under current filters
    cla(app.DCAxes); cla(app.RsAxes);
    cla(app.PkAmpAxes); cla(app.RmAxes);
...
% refresh variable list after subsetting
vars = T.Properties.VariableNames;
    ...
    % --------- Pulse number vector ----------
    % Find column whose name starts with 'Pul' (e.g. 'Pul_')
    ...
    % PkAmp axis (this one still shows labels on drug markers if you like)
    app.plotTimeParamOnAxis(app.PkAmpAxes, pkampParam, x, xLabel, pul, T);
    % ------------------------------------------------------

    % --------- OPTIONAL: Pulse2/Pulse1 ratio (unchanged) ----------
    if isprop(app, 'RatioAxes') && ismember('PkAmp', vars) && any(pul == 2)
        % group by Filename (one ratio per sweep)
        ...
        [G,~,gid] = unique(filename);      % G = unique sweep IDs
        nSweeps = numel(G);
        ...
        for k = 1:nSweeps
            ...
            if any(idx1) && any(idx2)
                ...
                if p1 ~= 0 && ~isnan(p1) && ~isnan(p2)
                    ...
                    % x for this sweep = mean of chosen x over its rows
                    ratioX(k) = mean(x(rows));
                end
            end
        end
   ...
   n = app.applyReanalysisRmToWorkbook();
    ...
    methods (Access = private)
        ...
        function createComponents(app)
            ...
            app.RecomputeButton.Position = [103 524 100 22];
    ...
    methods (Access = public)
        ...
        % Construct app
        function app = QuickPlots_exported
