...
This program was designed to quickly review recording performed with the WinLTP software (https://winltp.co.uk/) developed by Dr. Bill Anderson. 
After selecting the directory containing the data, the program features three tabs summarized as follows:

(1) Sweep Select

This tab allows you to view individual sweep traces (.P0 or .P1 files) and quickly create averages. Up to 4 averages may be created containing any number of sweeps, and these can be named according to user-defined periods (for example, control, drug, etc.) The average files can then be saved to an Excel spreadsheet for plotting in other programs.
...
This feature was added so that the user can quickly add indicators to the plots (and to the workbook) indicating which sweeps drugs were applied.  Up to 4 drug on/drug off options can be selected. 
